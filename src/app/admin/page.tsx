...
export default function Admin() {
  ...
  const exportCSV = () => {
    ...
    const csv = [
      headers.join(','),
      ...responses.map((row: any) => headers.map(h => JSON.stringify(row[h])).join(','))
    ].join('\n');
          ...
          <tbody>
            {responses.map((r: any, i: number) => (
              <tr key={i} className="border-b border-gray-700">
                ...
                <td className="p-2">{r.trial_number}</td>
                <td className="p-2">{r.adverb}</td>
                <td className="p-2">{r.emotion_x}, {r.emotion_y}</td>
                <td className="p-2">{r.motivation_x}, {r.control_y}</td>
                <td className="p-2">{r.berkeley_x}, {r.berkeley_y}</td>
                <td className="p-2">{r.imagination}</td>
